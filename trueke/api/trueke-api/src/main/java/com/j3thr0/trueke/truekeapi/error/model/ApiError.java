package com.j3thr0.trueke.truekeapi.error.model;
...
public interface ApiError {
    ...
    static ApiError fromErrorAttributes(Map<String, Object> defaultErrorAttributesMap){
        ...
        ApiErrorImpl result = ApiErrorImpl.builder()
                ...
                .message(
                        (String)defaultErrorAttributesMap.getOrDefault("message", "No message available")
                )
                .path(
                        (String)defaultErrorAttributesMap.getOrDefault("path", "No path available")
                )
                .build();
        if (defaultErrorAttributesMap.containsKey("errors")){
            ...
            List<ApiSubError> subErrors = errors.stream().map(ApiValidationSubError::fromObjectError)
